...
with open(str(Path(__file__).parent / "config.yml"), 'r', encoding='utf-8') as file:
...
# ***** 调试参数输入区 *****
# sys.argv.append("")
# sys.argv[1] = r"G:\Program Data\ONEDRIVE\987384390\OneDrive\新存储\PY-MYGOV\时存区\投资司时区"
# print(sys.argv)
# *************************
...
if not abstf_path.exists() or abstf_path.is_dir():
    sg.popup_error("%s.py 文件不存在！请检查目录是否正确" %
                   contentmenuex_file_name, font="微软雅黑")
    exit(-1)
...
AdvancedMenuEx.AdvancedMenuEx(menu_items).show_context_menu()
